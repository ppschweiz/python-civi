...
def assign_member_ids(members, dryrun):
	...
	sys.stderr.write('Highest assigned member id before updates is ' + str(high_member_id))
	...
	for member in members:
		if len(member.memberships) > 0 and member.member_id < 1:
			...
			if not dryrun:
				...
				sys.stderr.write'Assiging member id ' + str(member.member_id) + ' to contact ' + str(member.civi_id))
			else:
				sys.stderr.write'Not assiging member id ' + str(member.member_id) + ' to contact ' + str(member.civi_id) + ' in dryrun')

			send_message(member, 'welcome', dryrun)

	sys.stderr.write('Highest assigned member id after updates is ' + str(high_member_id))
